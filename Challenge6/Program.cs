...
namespace Challenge6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            ProductsQuantity();
            // Productos que cuestan menos de 12, ordenados por el más barato.
            ProductsPriceLessThan12();
            // Top 3 de productos más caros.
            Top3ExpensiveProducts();
            ...
            SortedByCategoryThenByPrice();
            // el producto con mayor cantidad de palabras
            TopQuantityWordsProduct();
            // el producto que su precio total sea el mayor multiplicando su precio unitario por la cantidad en stock
            TopPriceProductQuantityPerStock();
        }

        public static void FirstElement()
        {
            var result = Products.ProductList
                .Where(product => product.Category == "Seafood")
                .First();
            Console.WriteLine($"El primer elemento de la categoria 'Seafood' es {result.ProductName}");
        }
        ...
        public static void AveragePricePerCategory()
        {
            var result = Products.ProductList
                ...
                {
                    Category = s.Key,
                    AveragePrice = s.Average(product => product.UnitPrice)
                });

            Console.WriteLine($"Categoria y precio promedio.");
            foreach(var p in result)
            {
                Console.WriteLine($"Categoria: {p.Category} Precio Promedio: {p.AveragePrice}");
            }
        }

        public static void ProductsQuantity()
        {
            var result = Products.ProductList.Count();
            Console.WriteLine($"La cantidad total de productos es: {result}");
        }

        public static void ProductsPriceLessThan12()
        {
            var result = Products.ProductList
            ...
            Console.WriteLine("Los productos que cuestan menos de 12 ordenados por el más barato son:");
            foreach(var p in result)
            {
        ...
        public static void Top3ExpensiveProducts()
        {
            var result = Products.ProductList
            ...
            Console.WriteLine("Los 3 productos más caros son:");
            foreach(var p in result)
            {
        ...
        public static void Top3CheapProducts()
        {
            var result = Products.ProductList
                .OrderBy(product => product.UnitPrice)
                .Take(3)
            ...
            Console.WriteLine("Los 3 productos más baratos son:");
            foreach (var p in result)
            {
                Console.WriteLine($"- {p.Name} Price: {p.Price}");
            }
        ...
        public static void LongestProductName()
        {
            var result = Products.ProductList
                .OrderByDescending(product => product.ProductName.Length)
                .First();

            Console.WriteLine($"El producto con el nombre mas largo es {result.ProductName}");

        }

        public static void SumStockProducts()
        {
            var result = Products.ProductList
                .Sum(product => product.UnitsInStock);
            Console.WriteLine($"La cantidad total de productos en el stock es de: {result}");
        }
        ...
        public static void GetProductById(int id)
        {
            ...
            if (result == null)
                ...
                Console.WriteLine($"El producto con el ID {id} es {result.ProductName}");
        }

        public static void SortedByCategoryThenByPrice()
        {
            var result = Products.ProductList
                .OrderBy(product => product.Category)
                .ThenBy(product => product.UnitPrice);

            Console.WriteLine("Productos ordenados por categoria y luego por precio");
            foreach(var p in result)
            {
                Console.WriteLine($"Nombre: {p.ProductName} Categoria: {p.Category} Precio: {p.UnitPrice}");
            }
        }

        public static void TopQuantityWordsProduct()
        {
            var result = Products.ProductList
                .OrderByDescending(product => product.ProductName.Split().Length)
                .First();

            Console.WriteLine($"La palabra con mayor cantidad de palabras es: {result.ProductName}");
        }

        public static void TopPriceProductQuantityPerStock()
        {
            var result = Products.ProductList
                .OrderByDescending(product => product.UnitPrice * product.UnitsInStock)
                .First();

            Console.WriteLine($"El producto con el precio mayor multiplicando precio por unidades en stock es: {result.ProductName}");
        }
